...
describe('<UserSettingsThemeRadioGroup />', () => {
  it('toggles the theme with a radio option', async () => {
    const themeApi = apiRegistry.get(appThemeApiRef);
    const rendered = await renderWithEffects(
      wrapInTestApp(
        <ApiProvider apis={apiRegistry}>
          <UserSettingsThemeRadioGroup />
        </ApiProvider>,
      ),
    );
    ...
    const themeButton = rendered.getByLabelText('Mock Theme');
